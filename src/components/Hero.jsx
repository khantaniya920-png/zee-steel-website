...
export default function Hero({ onOpenContact }) {
  return (
    <section className="relative flex min-h-[86vh] items-end overflow-hidden pt-20">
      <PlaceholderImage
        name="hero-architectural-gate"
        label="Luxury custom steel gate"
      ...
      <div className="container-content relative z-10 pb-20 pt-16">
        ...
        >
          ...
          <div className="mt-8 flex flex-wrap gap-4">
            <Button to="/projects" variant="primary">Explore Our Work</Button>
            <Button onClick={onOpenContact} variant="outline">Get a Quote</Button>
          </div>

          <div className="mt-10 flex flex-wrap gap-x-8 gap-y-3 border-t border-white/15 pt-6 text-sm text-mist/70">
            <span className="flex items-center gap-2"><Calendar size={16} className="text-champagne" /> Established {site.established}</span>
            <span className="flex items-center gap-2"><MapPin size={16} className="text-champagne" /> Ghaziabad, Uttar Pradesh</span>
            <span className="flex items-center gap-2"><Globe2 size={16} className="text-champagne" /> {site.serviceArea}</span>
          </div>
        </motion.div>
      </div>
    </section>
